...
| 日期 | 2026-09-20 |
| 基础工程 | forge-1.20.1-mod-3（本工程，MC 1.20.1 / Forge 47.4.23 / MDG Legacy 工程化模板，`mod_id=nextcard`、主包 `com.klze.nextcard`） |
| 作者 | klze |
| 文档地位 | 想法 → 可行性验证 → 架构方向 → 三轮拍板记录 → 规格冻结。**规格本身不含代码**；M0 与 v1.1 已落地（core 六抽象 + 四道门 + 16 卡示例内容），M1 起的 MC 粘合见 §7 里程碑 |
